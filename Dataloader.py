...
class CustomDataset:
    def __init__(self, graphdics, current_label_list, loaded_label_list, loaded_mask_list,geo_point_list_train_dics):
        ...
        self.geo_point_list_train_dics=geo_point_list_train_dics
    def __len__(self):
        return len(self.graphdics[0])
    def __getitem__(self, index):
        graphdic = {k:v[index] for k,v in self.graphdics.items()}
        geo_point_list_train_dic={k:v[index] for k,v in self.geo_point_list_train_dics.items()}
        ...
        return graphdic, current_label, loaded_label, loaded_mask,geo_point_list_train_dic
...
class CustomDataLoader:
    ...
    def __next__(self):
        ...
        for b in batch_data:
            graphdic, current_label, loaded_label, loaded_mask,geo_point_list_train_dic=b
            for key in graphdic.keys():
                if key not in graphdics.keys():
                    graphdics[key]=[]
                    geo_point_list_train_dics[key]=[]
                graphdics[key].append(graphdic[key])
                geo_point_list_train_dics[key].append(geo_point_list_train_dic[key])
        ...
        return graphdics,current_labels,loaded_labels,loaded_masks,geo_point_list_train_dics
...
def get_dataloader(path, batch_size,days):
    loaded_data_list,loaded_label_list,loaded_mask_list,current_label_list,geolist,geo_point_list_train_list= torch.load(path)#n*14*tu,n*8*2,n*8
    graphdics={}
    geo_point_list_train_dics={}
    ...
    for  i in range(len(loaded_data_list)):
        for j in range(len(loaded_data_list[i])):
            ...
            geo_point_list_train_dics[j].append(geo_point_list_train_list[i][j])
    # 示例使用
    # 假设有一个包含7天图数据的字典以及相应的label列表和mask列表
    # 创建自定义数据集
    custom_dataset = CustomDataset(graphdics, current_label_list, loaded_label_list, loaded_mask_list,geo_point_list_train_dics)
    custom_dataloader = CustomDataLoader(custom_dataset, batch_size=batch_size, shuffle=True)
    return custom_dataloader,geolist